...
# primary key 존재하면 primary key 순
create table ex12(
...
select * from ex12;
show index from ex12;
insert into ex12 values(1,1,1);
insert into ex12 values(2,2,2);
insert into ex12 values(4,4,4);
insert into ex12 values(3,3,3);
...
delete from ex13 where num2=2;
select * from ex13;
select * from ex13 where num1>0;
...
insert into ex14 values (6,'클러스터형 인덱스는 인덱스에 맞춰서 행이 자동으로 정렬되고, 테이블당 하나만 사용할 수 있습니다. 반면에 보조 인덱스는 정렬기능은 따로 가지지 않고, 테이블당 여러개 사용할 수 있습니다.');

show index from ex14;
